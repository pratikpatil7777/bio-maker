...
export default function ShareButton({
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast.success(getText('Link copied!', 'लिंक कॉपी हो गई!', 'लिंक कॉपी झाली!'));
    } catch {
      await showError(
  ...
  const handlePrivacyConfirm = async (sharePrivately: boolean) => {
    ...
    try {
      ...
      const message = sharePrivately
        ? getText('Private link copied!', 'निजी लिंक कॉपी हो गई!', 'खाजगी लिंक कॉपी झाली!')
        : getText('Link copied!', 'लिंक कॉपी हो गई!', 'लिंक कॉपी झाली!');
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={isSharing}
        className={`h-8 xs:h-9 sm:h-9 flex items-center gap-1 xs:gap-1.5 px-2 xs:px-3 sm:px-4 rounded-lg font-semibold text-[10px] xs:text-xs transition-all duration-300 touch-target ${
          isSharing
      ...
      {showOptions && !isSharing && (
        <>
          ...
          <div className="absolute right-0 sm:right-0 left-auto top-full mt-2 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden z-[1001] min-w-[200px] max-w-[calc(100vw-16px)] sm:max-w-none">
            {/* Share via Web Share API - Touch-friendly */}
            <button
            ...
            >
              ...
              <span className="font-medium">{getText('Share Image', 'इमेज शेयर करें', 'इमेज शेअर करा')}</span>
            </button>
            ...
            >
              <div className="w-10 h-10 sm:w-8 sm:h-8 rounded-full bg-[#25D366] flex items-center justify-center flex-shrink-0">
            ...
            {/* Copy Link - Touch-friendly */}
            <button
              onClick={handleCopyLink}
              className="w-full flex items-center gap-3 px-4 py-4 sm:py-3 text-sm text-[#333] dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors cursor-pointer border-t border-gray-100 dark:border-slate-700 active:bg-gray-100 dark:active:bg-slate-600"
            >
              <div className="w-10 h-10 sm:w-8 sm:h-8 rounded-full bg-gray-100 dark:bg-slate-600 flex items-center justify-center flex-shrink-0">
              ...
              <span className="font-medium">{getText('Copy Page Link', 'पेज लिंक कॉपी', 'पेज लिंक कॉपी')}</span>
            </button>

            {/* Shareable Link with Privacy Options - Touch-friendly */}
            {biodataData && (
              <button
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="font-medium">{getText('Shareable Link', 'शेयर योग्य लिंक', 'शेअर करण्यायोग्य लिंक')}</span>
